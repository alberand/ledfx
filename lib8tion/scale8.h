...
LIB8STATIC_ALWAYS_INLINE uint8_t l8t_scale8( uint8_t i, l8t_fract8 scale)
{
#if SCALE8_C == 1
...
LIB8STATIC_ALWAYS_INLINE uint8_t l8t_scale8_video( uint8_t i, l8t_fract8 scale)
{
...
LIB8STATIC_ALWAYS_INLINE uint8_t l8t_scale8_LEAVING_R1_DIRTY( uint8_t i, l8t_fract8 scale)
{
#if SCALE8_C == 1
#if (FASTLED_SCALE8_FIXED == 1)
...
LIB8STATIC_ALWAYS_INLINE void l8t_nscale8_LEAVING_R1_DIRTY( uint8_t& i, l8t_fract8 scale)
{
#if SCALE8_C == 1
#if (FASTLED_SCALE8_FIXED == 1)
...
/// then explicitly call cleanup_R1.
LIB8STATIC_ALWAYS_INLINE uint8_t l8t_scale8_video_LEAVING_R1_DIRTY( uint8_t i, l8t_fract8 scale)
{
#if SCALE8_C == 1 || defined(LIB8_ATTINY)
    uint8_t j = (((int)i * (int)scale) >> 8) + ((i&&scale)?1:0);
    // uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
...
/// If you are doing several 'scale8_video's in a row, use this, and
/// then explicitly call cleanup_R1.
LIB8STATIC_ALWAYS_INLINE void l8t_nscale8_video_LEAVING_R1_DIRTY( uint8_t & i, l8t_fract8 scale)
{
#if SCALE8_C == 1 || defined(LIB8_ATTINY)
...
LIB8STATIC_ALWAYS_INLINE void l8t_cleanup_R1()
{
...
LIB8STATIC void l8t_nscale8x3( uint8_t& r, uint8_t& g, uint8_t& b, l8t_fract8 scale)
{
#if SCALE8_C == 1
#if (FASTLED_SCALE8_FIXED == 1)
...
#endif
#elif SCALE8_AVRASM == 1
    r = l8t_scale8_LEAVING_R1_DIRTY(r, scale);
    g = l8t_scale8_LEAVING_R1_DIRTY(g, scale);
    b = l8t_scale8_LEAVING_R1_DIRTY(b, scale);
    l8t_cleanup_R1();
#else
...
///
///         THIS FUNCTION ALWAYS MODIFIES ITS ARGUMENTS IN PLACE
LIB8STATIC void l8t_nscale8x3_video( uint8_t& r, uint8_t& g, uint8_t& b, l8t_fract8 scale)
{
#if SCALE8_C == 1
    uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
...
#elif SCALE8_AVRASM == 1
    l8t_nscale8_video_LEAVING_R1_DIRTY( r, scale);
    l8t_nscale8_video_LEAVING_R1_DIRTY( g, scale);
    l8t_nscale8_video_LEAVING_R1_DIRTY( b, scale);
    l8t_cleanup_R1();
#else
#error "No implementation for nscale8x3 available."
#endif
...
LIB8STATIC void l8t_nscale8x2( uint8_t& i, uint8_t& j, l8t_fract8 scale)
{
#if SCALE8_C == 1
#if FASTLED_SCALE8_FIXED == 1
    uint16_t scale_fixed = scale + 1;
...
#endif
#elif SCALE8_AVRASM == 1
    i = l8t_scale8_LEAVING_R1_DIRTY(i, scale);
    j = l8t_scale8_LEAVING_R1_DIRTY(j, scale);
    l8t_cleanup_R1();
#else
...
LIB8STATIC void l8t_nscale8x2_video( uint8_t& i, uint8_t& j, l8t_fract8 scale)
{
#if SCALE8_C == 1
    uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
...
#elif SCALE8_AVRASM == 1
    l8t_nscale8_video_LEAVING_R1_DIRTY( i, scale);
    l8t_nscale8_video_LEAVING_R1_DIRTY( j, scale);
    l8t_cleanup_R1();
#else
#error "No implementation for nscale8x2 available."
#endif
...
LIB8STATIC_ALWAYS_INLINE uint16_t l8t_scale16by8( uint16_t i, l8t_fract8 scale )
{
#if SCALE16BY8_C == 1
    uint16_t result;
#if FASTLED_SCALE8_FIXED == 1
...
LIB8STATIC uint16_t l8t_scale16( uint16_t i, l8t_fract16 scale )
{
  #if SCALE16_C == 1
    uint16_t result;
#if FASTLED_SCALE8_FIXED == 1
...
LIB8STATIC uint8_t l8t_dim8_raw( uint8_t x)
{
    return l8t_scale8( x, x);
}

/// Adjust a scaling value for dimming for video (value will never go below 1)
LIB8STATIC uint8_t l8t_dim8_video( uint8_t x)
{
    return l8t_scale8_video( x, x);
}
...
LIB8STATIC uint8_t l8t_brighten8_raw( uint8_t x)
{
    uint8_t ix = 255 - x;
    return 255 - l8t_scale8( ix, ix);
}

/// inverse of the dimming function, brighten a value
LIB8STATIC uint8_t l8t_brighten8_video( uint8_t x)
{
    uint8_t ix = 255 - x;
    return 255 - l8t_scale8_video( ix, ix);
}

/// inverse of the dimming function, brighten a value
LIB8STATIC uint8_t l8t_brighten8_lin( uint8_t x )
{
    uint8_t ix = 255 - x;
    if( ix & 0x80 ) {
        ix = l8t_scale8( ix, ix);
